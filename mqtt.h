...
void updateMQTT(String mqtt_str,String val_s){
  ...
  byte j = mqtt_str.lastIndexOf("/");

  if(j>0 && mqtt_str.length()-4<j){ //что б ограничить 2-мя знаками 
    String mqtt_str08 = mqtt_str.substring(0, j);
    j= StringToInt( mqtt_str.substring(j+1));
...
void mqtt_init(){
  ...
  const char *n;
  if(config.mqtt_name!=""){
    n=StringToChar(config.mqtt_name);
    Serial.println(config.mqtt_name);
  }else{
  ...
  if(config.mqtt_user!=""){
    const char *mqtt_user = StringToChar(config.mqtt_user);
    if(config.mqtt_pass!=""){
      const char *mqtt_pass = StringToChar(config.mqtt_pass);
      mqtt.connect(n, mqtt_user, mqtt_pass);  
    }else{
      mqtt.connect(n, mqtt_user);  
    }
  }else{
    mqtt.connect(n);   
  }
